...
export default class DropboxClient {
  ...
  constructor(options: DropboxOptions = {}) {
    ...
    this.clientSecret = String(
      options.clientSecret ?? process.env.DBX_CLIENT_SECRET
    );
    this.refreshToken = String(
      options.refreshToken ?? process.env.DBX_REFRESH_TOKEN
    );
  ...
  private canRefreshToken(): boolean {
    ...
    return (
      Date.now() >=
      this.accessTokenExpiration - this.accessTokenExpirationBuffer
    );
  }
  ...
  private async refreshAccessToken(): Promise<void> {
    ...
    const response = await this.executeRequest(
      DropboxDomains.auth.refreshToken,
      {
        method: "POST",
        headers: this.getHeaderOptions({}, false),
        body: this.buildRequestData({
  ...
  public async download(
    ...
    const dropboxApiResponse = JSON.parse(
  ...
  public async upload(
    ...
    const response = await this.executeRequest(
      `${DropboxDomains.files.content}/upload`,
      {
        method: "POST",
        headers: this.getFileHeaderOptions(path, {
          autorename: false,
  ...
  public async createSharedLink(
    ...
    const response = await this.executeRequest(
      `${DropboxDomains.sharing.sharedLink}/create_shared_link_with_settings`,
      {
        method: "POST",
    ...
    if (!response.ok) {
      const { error_summary, error } =
        response.data as DropboxSharedLinkResultError;
  ...
  public async revokeSharedLink(url: string): Promise<string> {
    ...
    const response = await this.executeRequest(
      `${DropboxDomains.sharing.sharedLink}/revoke_shared_link`,
      {
        method: "POST",
        headers: this.getHeaderOptions({ "Content-Type": "application/json" }),
  ...
  public async getSharedLink(
    path: string
  ): Promise<DropboxGetSharedLinkResult> {
    await this.validateAccessToken();

    const response = await this.executeRequest(
      `${DropboxDomains.sharing.sharedLink}/list_shared_links`,
      {
        method: "POST",
        headers: this.getHeaderOptions({ "Content-Type": "application/json" }),
  ...
  public async getFileMetadata(path: string): Promise<DropboxFileMetadata> {
    ...
    const response = await this.executeRequest(
      `${DropboxDomains.files.file}/get_metadata`,
      {
        method: "POST",
        headers: this.getHeaderOptions({ "Content-Type": "application/json" }),
        body: this.JSONHeader({ path }),
